...
class ProjectionBuffer:
    ...
    def create_train_data(self,
        ...
        state_dim = rollout_full_states.shape[-1]
        ...
        cluster_full_states = np.zeros([self.n_train_data, data_nr_timesteps, state_dim])
        ...
        for i_trajectory in range(self.n_train_data):
            ...
            cluster_observs[i_trajectory, :steps_next_start, :] = rollout_observations[idx_timestep:idx_trajectory_end, idx_envs, :]
            cluster_mask[i_trajectory, :steps_next_start] = True
            cluster_full_states[i_trajectory, :steps_next_start, :] = rollout_full_states[idx_timestep:idx_trajectory_end, idx_envs, :]
        ...
        self.train_data = {
                ...
                'full_states': cluster_full_states[:, :max_timesteps, :]
        }
        # shape: [num_timestep, num_env, :]
    ...
    def train_step(self, train_data_value: np.ndarray) -> float:
        ...
        improvement_begin_to_step = self.policy_improvement(train_data_value) # improv.shape = [num_trajectory]
